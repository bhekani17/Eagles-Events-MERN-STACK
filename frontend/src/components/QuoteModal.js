...
export function QuoteModal({ isOpen, onClose, preSelectedItem = null }) {
  ...
  const [feedbackPrefill, setFeedbackPrefill] = useState({ name: '', email: '' });
  ...
  }, [isOpen, fetchEquipment, fetchPackages]);
      ...
      const maybeAddPreselected = async () => {
        try {
          ...
          if (preSelectedItem.type === 'package' && preSelectedItem.packageId) {
            const pkgResponse = await publicAPI.getPackage(preSelectedItem.packageId);
            ...
            setFormData(prev => {
              ...
              const pkgItem = {
                ...
                price: (pkg.price ?? pkg.basePrice ?? preSelectedItem.estimatedCost ?? 0),
                quantity: 1,
                maxQuantity: 1,
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">
                Your exact event type selection will be stored in the database.
              </p>
            </div>
          </div>
          ...
          <div className="sticky top-6 bg-gray-50 border border-gray-200 rounded-lg p-4">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-medium text-gray-900">{formData.selectedItems.length}</span>
              </div>
                  ...
                  <div className="space-y-1 max-h-48 overflow-y-auto pr-1">
                    {formData.selectedItems.map((item) => (
                      <div key={item.id} className="flex justify-between text-sm">
                        ...
                        <span className="font-medium">{formatPrice(item.price)}/day</span>
                      </div>
                    ))}
                  </div>
                </div>
      ...
      <PaymentModal
        ...
        quoteData={formData}
        onSuccess={({ quoteData }) => {
          // Prefill feedback from submitted quote data and show feedback modal
          setFeedbackPrefill({ name: quoteData?.name || '', email: quoteData?.email || '' });
      ...
      <FeedbackModal
        ...
        defaultName={feedbackPrefill.name}
        defaultEmail={feedbackPrefill.email}
      />
    </div>
  );
}
