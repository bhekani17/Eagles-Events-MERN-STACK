...
export function PaymentModal({ isOpen, onClose, quoteData, onSuccess, onBack }) {
  ...
  const handleClose = () => {
    ...
    setIsComplete(false);
    // If the flow completed successfully, notify parent before closing
    if (wasComplete && typeof onSuccess === 'function') {
      try {
        onSuccess({
          reference,
  ...
  const handleSubmitPayment = async (e) => {
    ...
    try {
      ...
      const quoteDataToSubmit = {
        ...
        eventTypeOther: quoteData.eventType === 'other' ? (quoteData.eventTypeOther || '') : null,
        ...
        <div className="bg-white rounded-2xl max-w-md w-full p-8 text-center">
            ...
            <div className="mb-6">
              ...
              {selectedPayment === 'eft' && (
                <p className="text-xs text-gray-600 mt-2">
                  Use the reference <span className="font-semibold">{reference}</span> when making your EFT so we can match your payment.
                </p>
              )}
            </div>
          )}
          {selectedPayment === 'eft' && (
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6 text-left">
              <div className="font-medium text-blue-900 mb-2">EFT Banking Details</div>
              <div className="text-sm text-blue-800 space-y-1">
                ...
                <p className="pt-1"><strong>Reference:</strong> Use <span className="font-semibold">{reference || 'the generated reference above'}</span> when making your EFT. Please WhatsApp or email the Proof of Payment (POP) and update.</p>
              </div>
            </div>
          )}
          <div className="bg-gold-50 border border-gold-200 rounded-lg p-4 mb-6">
            ...
            <p className="text-sm text-gold-700">
              Email sent to {quoteData?.email} and admin team{reference ? ` with reference ${reference}` : ''}.
            </p>
          </div>
          <button
            onClick={handleClose}
      ...
      <div className="bg-white rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
        <div className="relative p-6 border-b border-gray-200">
          ...
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            <div className="text-center">
              ...
              <div className="text-sm text-gray-500">Step 2: Payment</div>
            </div>
          </div>
          {/* Controls layer */}
          <div className="flex items-center justify-between">
            <button
              ...
              className="inline-flex items-center gap-2 px-3 py-1.5 border border-gray-200 rounded-lg text-gray-700 hover:bg-gray-50"
              aria-label="Back"
            >
        ...
        <div className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {/* Left: Payment form */}
            <form onSubmit={handleSubmitPayment} className="md:col-span-2">
                ...
                {paymentOptions.map((option) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center mb-2">
                        <IconComponent className={`w-6 h-6 ${selectedPayment === option.id ? 'text-gold-600' : 'text-gray-600'} mr-3`} />
            ...
            <div className="md:col-span-1">
              <div className="sticky top-6 bg-gray-50 border border-gray-200 rounded-lg p-4">
