    ...
    try {
      ...
      const customersWithQuotes = await Promise.all(
        customersData.map(async (customer) => {
          try {
            // Only fetch quotes if customer has a valid email
            if (!customer.email || customer.email === 'No email' || customer.email.trim() === '') {
              return {
                ...customer,
                quotes: [],
                quotesCount: 0,
            ...
            // Fetch quotations for this customer by email (exact match)
            const quotesResponse = await adminAPI.getQuotes({ 
              customerEmail: customer.email.trim(),
              limit: 50 // Increased limit to get all quotes for status checking
            });
            
            const quotes = quotesResponse?.data || [];
            
            // Additional validation: ensure quotes actually belong to this customer
            // Only count quotes where email matches exactly
            const validQuotes = quotes.filter(quote => 
              quote.email && 
              quote.email.trim().toLowerCase() === customer.email.trim().toLowerCase()
            );
            
            // Only count customers if they have at least one approved quote
            const approvedQuotes = validQuotes.filter(quote => 
              quote.status && quote.status.toLowerCase() === 'approved'
            );
            
            const quotesCount = approvedQuotes.length;
            
            // Calculate total spent from approved quotes only
            const totalSpent = approvedQuotes.reduce((sum, quote) => {
              return sum + (parseFloat(quote.totalAmount || quote.amount || 0));
            }, 0);
            
            return {
              ...customer,
              quotes: approvedQuotes, // Only store approved quotes
              quotesCount,
              totalSpent,
              hasApprovedQuotes: approvedQuotes.length > 0
            };
          } catch (quoteErr) {
            console.warn(`Failed to fetch quotes for customer ${customer.email}:`, quoteErr);
            return {
              ...customer,
              quotes: [],
              quotesCount: 0,
      ...
      const processedCustomers = customersWithQuotes
        .filter(customer => customer.hasApprovedQuotes) // Only show customers with approved quotes
        .map(customer => ({
          ...customer,
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600 mt-1">Showing only customers with approved quotes</p>
        </div>
        <button
          onClick={() => setShowAddModal(true)}
          className="bg-amber-400 text-black px-4 py-2 rounded-lg hover:bg-amber-300 font-medium flex items-center focus:outline-none focus:ring-2 focus:ring-amber-500 focus:ring-offset-2"
        >
      ...
      <div className="bg-blue-50 border border-blue-200 text-blue-700 px-4 py-3 rounded-lg mb-4">
        <div className="flex items-start justify-between">
          <div className="flex items-start">
            ...
            <div>
              <p className="font-medium">Customer Display Rules:</p>
              <p className="text-sm mt-1">
                <strong>Approved Quotes Only:</strong> Only customers with approved quotes are displayed in this list.
              </p>
              <p className="text-sm mt-1">
                <strong>Email Matching:</strong> Quotes are only counted when email addresses match exactly. 
                This ensures accurate customer relationship tracking and prevents duplicate counting.
              </p>
            </div>
          </div>
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
            ...
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                ...
                <div className="flex items-center">
                  ...
                  Approved Quotes
                  <div className="ml-1 text-xs text-gray-400 font-normal">
                    (by email)
                  </div>
                </div>
              </th>
              ...
              <tr key={customer._id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  ...
                  <div className="text-center">
                    <div className="font-medium text-blue-600 text-lg">{customer.quotesCount || 0}</div>
                    <div className="text-xs text-gray-500">
                      {(customer.quotesCount || 0) === 1 ? 'quote' : 'quotes'}
                    </div>
                    {(customer.quotesCount || 0) > 0 && (
                      <div className="text-xs text-green-600 font-medium">
                        {customer.quotes && customer.quotes.length > 0 ? 'Active' : 'Pending'}
                      </div>
                    )}
                  </div>
                </td>
                 <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                   <div className="space-y-1">
                     {customer.quotes && customer.quotes.length > 0 ? (
                       <>
                         {customer.quotes.slice(0, 3).map((quote, idx) => (
                           <div key={idx} className="text-xs flex items-center justify-between">
                             <span className="text-gray-600">Quote {idx + 1}:</span>
                             <span className="ml-2 font-medium text-green-600">
                               {formatCurrency(quote.totalAmount || quote.amount || quote.price || 0)}
                             </span>
                           </div>
                         ))}
                         {customer.quotes.length > 3 && (
                           <div className="text-xs text-gray-500 text-center pt-1 border-t border-gray-200">
                             +{customer.quotes.length - 3} more quotes
                           </div>
                         )}
                         <div className="text-xs text-blue-600 font-medium text-center pt-1 border-t border-gray-200">
                           Total: {formatCurrency(customer.totalSpent || 0)}
                         </div>
                       </>
                     ) : (
                       <div className="text-center">
                         <span className="text-gray-400 text-xs">No quotes</span>
                         <div className="text-xs text-gray-300">yet</div>
                       </div>
                     )}
                   </div>
                 </td>
        ...
        {filteredAndSortedCustomers.length === 0 && (
          <div className="text-center py-8">
            ...
            <p className="text-gray-500">No customers with approved quotes found</p>
            <p className="text-sm text-gray-400 mt-2">
              Only customers who have approved quotes are displayed in this list
            </p>
          </div>
        )}
      </div>
          ...
          <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
            <div>
              <p className="text-sm text-gray-700">
                ...
                </span> of <span className="font-medium">{totalItems}</span> customers with approved quotes
              </p>
            </div>
            <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-amber-50 p-4 rounded-lg">
              <div className="flex items-center">
                ...
                <div>
                  <p className="text-2xl font-bold text-amber-900">{customer.quotesCount || 0}</p>
                  <p className="text-sm text-amber-700">Total Quotes</p>
                </div>
              </div>
            </div>
      ...
      <div className="bg-white rounded-lg max-w-md w-full">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="flex justify-end space-x-3 pt-6 border-t border-gray-200">
            ...
            <button
              type="submit"
              className="px-4 py-2 bg-amber-400 text-black rounded-lg hover:bg-amber-300 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:ring-offset-2"
            >
              Update Customer
            </button>
          </div>
