...
const EquipmentManagement = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingEquipment) {
        await axios.put(`${API_URL}/${editingEquipment._id}`, cleanData, buildAuthConfig());
      } else {
        await axios.post(API_URL, cleanData, buildAuthConfig());
      }
            ...
            <div className="flex justify-center gap-4">
              {(searchTerm || categoryFilter !== 'all') ? (
                ...
                <button
                  ...
                  className="px-6 py-2 bg-amber-400 text-black rounded-md hover:bg-amber-300 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:ring-offset-2 flex items-center gap-2"
                >
                  <Plus size={18} />
            ...
            <div key={item._id} className="bg-white rounded-lg shadow-sm overflow-hidden border border-gray-200">
              ...
              <div className="p-4">
                ...
                <p className="text-gray-600 text-sm mb-2 line-clamp-2">
                  {item.description || item.features?.slice(0, 2).join(' • ') || 'No description'}
                </p>
                {Array.isArray(item.features) && item.features.length > 0 && (
                  <div className="flex flex-wrap gap-2 mb-3">
                    {item.features.slice(0, 4).map((feat, idx) => (
                      <span key={idx} className="px-2 py-0.5 text-[11px] rounded-full bg-gray-100 text-gray-700 border border-gray-200">
                        {feat}
                      </span>
                    ))}
                  </div>
                ...
                <div className="mt-3 grid grid-cols-2 gap-2 text-xs text-gray-600">
                  <div className="flex items-center gap-2">
                    <span className="inline-flex items-center px-2 py-0.5 rounded bg-gray-100 text-gray-700 border border-gray-200">Qty: {item.quantity ?? 1}</span>
                  </div>
                  <div className="flex items-center justify-end">
                    <span className={`px-2 py-0.5 rounded text-xs font-medium border ${item.availability !== false ? 'bg-green-50 text-green-700 border-green-200' : 'bg-red-50 text-red-700 border-red-200'}`}>
                      {item.availability !== false ? 'Available' : 'Unavailable'}
                    </span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="px-2 py-0.5 rounded bg-gray-100 text-gray-700 border border-gray-200">Images: {Array.isArray(item.images) ? item.images.length : 0}</span>
                  </div>
                  <div className="text-right text-gray-500">
                    Updated: {(() => { const d = item.updatedAt || item.createdAt; return d ? new Date(d).toLocaleDateString() : 'N/A'; })()}
                  </div>
                </div>
              </div>
            </div>
