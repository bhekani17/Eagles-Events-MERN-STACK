...
export const sendEmail = async (...args) => {
  try {
    ...
    const mailOptions = {
      from: `"${companyConfig.name}" <${process.env.EMAIL_FROM || process.env.EMAIL_USER}>`,
      to,
      subject,
      text,
      html,
    ...
    if (attachments && attachments.length > 0) {
      console.log('Email service: Attachments found:', attachments.length);
      ...
      console.log('Email service: No attachments found');
    }
