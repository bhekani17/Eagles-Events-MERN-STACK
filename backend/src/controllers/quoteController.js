...
export const submitQuote = async (req, res) => {
  try {
    ...
    const quoteData = {
      ...
      eventType: eventType,
      eventTypeOther: eventType === 'other' ? (eventTypeOther || '') : null,
...
async function sendAdminApprovalEmail(quote) {
  ...
  const subject = `✅ Quote Approved #${quote.reference || quote._id}`;
...
Quote ID: ${quote.reference || quote._id}
Name: ${quote.customerName}${quote.company ? ` (${quote.company})` : ''}
Email: ${quote.email}
Phone: ${quote.phone}
...
View in Admin: ${(process.env.FRONTEND_URL || 'https://eaglesevents.co.za')}/admin/quotes/${quote._id}
  ...
  const html = `
    ...
    <p><strong>Quote ID:</strong> ${quote.reference || quote._id}</p>
...
async function sendQuoteConfirmationEmail(quote, pdfBuffer, pdfFileName) {
  const subject = `Your Quote Request #${quote.reference || quote._id} Has Been Received`;
  const eventDate = new Date(quote.eventDate).toLocaleDateString('en-US', {
    year: 'numeric',
    month: 'long',
    day: 'numeric',
...
Website: ${companyConfig.website.replace('https://', '')}
  `;
    ...
    if (emailOptions.attachments) {
      emailOptions.attachments.forEach((attachment, index) => {
        console.log(`Attachment ${index + 1}:`, {
          ...
          contentLength: attachment.content ? attachment.content.length : 'null'
        });
      });
    }
...
async function sendAdminNotification(quote, pdfBuffer, pdfFileName) {
  ...
  const adminEmail = Array.isArray(adminEmailsRaw)
    ...
    : String(adminEmailsRaw)
        ...
        .filter(Boolean)
        .join(',');
  const subject = `📋 New Quote Request #${quote.reference || quote._id}`;
  const eventDate = new Date(quote.eventDate).toLocaleDateString('en-US', {
    year: 'numeric',
    month: 'long',
    day: 'numeric',
...
async function sendQuoteStatusUpdateEmail(quote) {
  const subject = `Your Quote #${quote.reference || quote._id} Has Been Confirmed`;
  const eventDate = new Date(quote.eventDate).toLocaleDateString('en-US', {
    year: 'numeric',
    month: 'long',
    day: 'numeric',
...
export const getQuotes = async (req, res) => {
  try {
    ...
    // Search by customer name, email, phone, or reference
    if (search) {
